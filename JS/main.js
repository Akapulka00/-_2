...
const
    ...
    E = [// часть функции F сблок расширения
        32, 1, 2, 3, 4, 5,
        4, 5, 6, 7, 8, 9,
        8, 9, 10, 11, 12, 13,
        12, 13, 14, 15, 16, 17,
    ...
    S = [// часть функции F кодирования 8 S блоков подстановки получающие 6 бит
        [
    ...
    P = [// часть функции F блок перестановки
        16, 7, 20, 21,
        29, 12, 28, 17,
        1, 15, 23, 26,
        5, 18, 31, 10,
...
const keySchedule = (key) => {//Функция подготовки ключа к записи
    let subkeys = [];
    let perm = PC1.map(index => key[index - 1]).join("");//создаем выделяем из массива 56 бит* (по ощущения выкинули 2 бита)
...
const expandBlock = block => E.map(index => block[index - 1]).join("");// расширитель Е
...
const des = (msg, key, subkeys) => {
    let perm = IP.map(index => msg[index - 1]).join(""); // начальная перестановка P
    let L0 = perm.substr(0, perm.length / 2);
    let R0 = perm.substr(perm.length / 2);

    let prevL0 = L0, prevR0 = R0;// записываем прошлые значения боков
    for (let i = 0; i < 16; i++) {
        L0 = prevR0;
...
//hex2a('32343630');//преобразование 16 ричного сообщения в аски
//a2hex('2460');//преобразование аски сообщения в 16 ричное
...
/// входные данные
//console.log("Vadim HI");
//console.log(a2hex('Vadim HI').toUpperCase());//процесс кодировки сообщения в 16 ричное
//console.log(hex2a('566164696D204849'));//процесс декодировки сообщения в  аски
let kluch="133457799BBCDFF1";
let text='Vadim HI';
// важно на вход поступают блоки по 8 байт (8 симвалов чар)
let key = bin(kluch); // hex ключ
let msg = bin(a2hex(text).toUpperCase()); // hex сообщение
let enc = encode(msg, key);
let  dec=decode(bin(enc), key);
console.log(enc); // => 85E813540F0AB405
console.log(hex2a(dec)); // => 0123456789ABCDEF

let text_fist=document.querySelector(".text_first");
text_fist.innerHTML="<span>Шифруемый текст:</span>"+text + "<pre><span>Ключ: </span>"+kluch;
let ecode_des=document.querySelector(".DES");
ecode_des.innerHTML="<span>Зашифрованный текст:</span>"+enc ;
let Decode_des=document.querySelector(".EN_DES");
Decode_des.innerHTML="<span>Расшифрованный текст:</span>"+hex2a(dec) ;